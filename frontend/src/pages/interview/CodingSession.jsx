...
export default function CodingSession() {
  ...
  const handleSubmitCode = async () => {
    ...
    try {
      ...
      setNetworkError(
        err.response?.data?.message || 'Failed to evaluate code. Please try again.'
      );
    } finally {
      setIsRunning(false);
    }
  ...
  if (loadingInitial) {
    return (
      <div className="min-h-screen bg-[#0E0E10] text-white flex flex-col items-center justify-center p-4">
        ...
        <h2 className="text-xl font-bold font-heading mb-2">Loading Coding IDE...</h2>
        <p className="text-xs text-neutral-400 max-w-sm text-center">
          Initializing problem constraints, test runner, and evaluation environment.
        </p>
      </div>
    );
  }
