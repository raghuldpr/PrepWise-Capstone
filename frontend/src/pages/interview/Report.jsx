...
export default function Report() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0E0E10] text-white flex flex-col items-center justify-center p-4">
        <div className="w-16 h-16 rounded-2xl bg-[#C85232]/15 border-2 border-[#C85232] flex items-center justify-center text-[#C85232] mb-4 animate-pulse">
          <Sparkles size={28} />
        </div>
        <h2 className="text-xl font-bold font-heading mb-2">Generating AI Performance Report...</h2>
        <p className="text-xs text-neutral-400 max-w-sm text-center">
          Synthesizing speech clarity, code correctness, technical depth, and actionable career insights.
        </p>
      </div>
    );
  }
  ...
  if (error || !report) {
    return (
      <div className="min-h-screen bg-[#0E0E10] text-white flex flex-col items-center justify-center p-4">
        <div className="max-w-md p-6 bg-[#18181B] rounded-2xl border border-red-500/30 text-center space-y-4 shadow-xl">
          <ShieldAlert size={40} className="text-red-400 mx-auto" />
          <h2 className="text-lg font-bold">Report Loading Error</h2>
          <p className="text-xs text-neutral-400 leading-relaxed">{error || 'Report data not found.'}</p>
